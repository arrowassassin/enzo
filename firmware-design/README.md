...
## The ten decisions in one screen
...
7. Sideloading is a **device-hosted Drop page** with two QR codes, streamed resumable uploads, plus an iOS Shortcut, an Android PWA share target, OPDS, Calibre wireless and WebDAV. No app required, ever.
8. OTA from SD and from GitHub plus a recovery slot from day one, because USB-locked units exist.
9. UI is print-like 1-bit, page-based, key-driven, with inversion as focus; the design brief in 07 encodes this.
10. Everything portable compiles and tests on the host, and a desktop simulator renders the real framebuffer.

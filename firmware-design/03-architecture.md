...
| Heavy formats | PDF is parsed on the device (own streaming parser: objects, xref and object streams, filters, fonts and encodings, content-stream interpretation, XY-cut reflow, images) within the device memory limits; the off-device converter (WASM in the browser and a CLI) produces `.qbk` for DjVu, DOCX and friends, and for PDFs the device declines (encrypted, JPX/JBIG2 scans, pathological files) | DjVu, DOCX and friends have no realistic no_std parser and would not fit in RAM anyway. |
...
- `/.quire/stats/sessions.log`: append-only binary records (postcard): book id, start (RTC), end, pages turned, characters read, orientation. One record per session; a session ends after 5 min idle or on book change. Per-page time is capped at 120 s and ignored under 5 s when computing pace.
- `/.quire/stats/daily.idx`: a rolling per-day summary (minutes, pages, characters, sessions, books touched) rebuilt from the log when missing, so the Analytics screens read a few hundred bytes, never the whole log.
- Forecasts ("Finish by Thursday") use characters per minute from the last seven sessions of that book, falling back to the global pace.
- The phone Analytics tab fetches `sessions.log` and `daily.idx` through the Drop page API and renders the rich charts client-side; exports (CSV, JSON, StoryGraph, Goodreads) are generated in the browser, not on the device.

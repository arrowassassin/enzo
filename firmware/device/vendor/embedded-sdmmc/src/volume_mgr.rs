    ...
    T: TimeSource,
{
    ...
    ///
    /// The name is matched case-insensitively against long and short names.
    /// If the file does not exist and `mode` allows creation, it is created:
    /// a unique 8.3 short name is generated (upper-cased, invalid characters
    /// replaced by `_`, with a `~n` numeric tail when needed) and the long
    /// file name entries are written in front of the short entry. Names which
    /// are already valid 8.3 names (ignoring case) are stored without long
    /// name entries, unless they mix upper and lower case.
    ///
    /// <div class="warning">
    ///
    /// This function gives you a [`RawFile`] and when you are finished with
    ...
    pub fn open_long_name_file_in_dir(&self, directory: RawDirectory, name: &str, mode: Mode) -> Result<RawFile, Error<D::Error>> {
        ...
        match mode {
            Mode::ReadWriteCreate => {
                ...
                let dir_info = data.open_dirs[directory_idx].clone();
                let att = Attributes::create_from_fat(0);
            ...
            _ => {
                // Safe to unwrap, since we actually have an entry if we got here
                let dir_entry = dir_entry.unwrap();
                self.open_existing_file(data, volume_idx, dir_entry, mode)
            }
        }
    }
...
// ****************************************************************************
//
// Long File Name support (Quire additions)
//
// ****************************************************************************
...
where
    ...
    T: TimeSource,
{
    /// Look in a directory for an entry with the given long file name.
    ///
    /// The name is compared case-insensitively against both the long file
    /// name and the 8.3 short name of every entry, so `"My Book.epub"`,
    /// `"my book.EPUB"` and the generated short name `"MYBOOK~1.EPU"` all find
    /// the same entry.
    ///
    /// You can just drop the [`DirEntry`] when you are done with it - it's a
    /// stand-alone object and takes up no resources in this Volume Manager.
    ...
    /// Open a directory with the given long file name, in the given directory.
    ///
    /// The name is matched case-insensitively against long and short names.
    /// Passing `"."` (or an empty string) opens `parent_dir` a second time and
    /// `".."` opens its parent.
    ///
    /// <div class="warning">
    ///
    ...
    /// Create a directory in a given directory, with the given long file name.
    ///
    ...
    pub fn make_long_name_dir_in_dir(&self, directory: RawDirectory, name: &str) -> Result<(), Error<D::Error>> {
        ...
        let parent_directory_idx = data.get_dir_by_id(directory)?;
        let volume_idx = data.get_volume_by_id(data.open_dirs[parent_directory_idx].raw_volume)?;
        let parent_info = data.open_dirs[parent_directory_idx].clone();
    ...
    /// Delete a closed file or an empty directory with the given long file
    /// name.
    ///
    /// The short entry and all of its long file name entries are marked as
    ...
    ///
    /// `from` and `to` are long file names. The entry keeps its first
    /// cluster, size, attributes and timestamps; only the directory entries
    ...
    pub fn rename_long_name_in_dir(&self, directory: RawDirectory, from: &str, to: &str) -> Result<(), Error<D::Error>> {
        ...
        let dir_idx = data.get_dir_by_id(directory)?;
        let volume_idx = data.get_volume_by_id(data.open_dirs[dir_idx].raw_volume)?;
        data.move_entry(dir_idx, &from, dir_idx, &to, volume_idx)
    }

    /// Move (and possibly rename) an entry from one directory to another on
    /// the same volume.
    ///
    /// `from` and `to` are long file names. If both directories are the same
    ...
    pub fn move_long_name(&self, dir_from: RawDirectory, from: &str, dir_to: RawDirectory, to: &str) -> Result<(), Error<D::Error>> {
        ...
        let from_idx = data.get_dir_by_id(dir_from)?;
        let to_idx = data.get_dir_by_id(dir_to)?;
        if data.open_dirs[from_idx].raw_volume != data.open_dirs[to_idx].raw_volume {
            return Err(Error::Unsupported);
        }
        let volume_idx = data.get_volume_by_id(data.open_dirs[from_idx].raw_volume)?;
        data.move_entry(from_idx, &from, to_idx, &to, volume_idx)
    }

    /// Open an existing file described by `dir_entry` (shared by the short
    /// and long name open functions).
    fn open_existing_file(
        &self,
        data: &mut VolumeManagerData<D, MAX_DIRS, MAX_FILES, MAX_VOLUMES>,
        volume_idx: usize,
        dir_entry: DirEntry,
        mode: Mode,
    ) -> Result<RawFile, Error<D::Error>> {
        let volume_id = data.open_volumes[volume_idx].raw_volume;
        ...
        // Remember this open file - can't be full as the caller checked already
        unsafe {
            data.open_files.push_unchecked(file);
        }
...
{
    /// Delete the entry `dir_entry` (found in the directory `dir_idx`): its
    /// short entry, its LFN entries and its cluster chain.
    ///
        ...
        match &mut self.open_volumes[volume_idx].volume_type {
            VolumeType::Fat(fat) => {
                fat.delete_long_name_entries(
                    ...
                    (dir_entry.entry_block, dir_entry.entry_offset),
                )?;
                fat.free_cluster_chain(&mut self.block_cache, dir_entry.cluster)?;
            }
        }
        Ok(())
    }

    /// Move the entry called `from` in directory `from_idx` to directory
    /// `to_idx` under the name `to`. Both directories must be on volume
    /// `volume_idx`. Used for renames too (`from_idx == to_idx`).
    ///
    /// The new entries are written before the old ones are removed, so a
    /// power loss in between leaves a duplicate entry rather than a lost
    /// file.
    fn move_entry(
        &mut self,
        from_idx: usize,
        ...
        let from_info = self.open_dirs[from_idx].clone();
        let to_info = self.open_dirs[to_idx].clone();
        ...
        if !entry.attributes.is_directory() && self.file_is_open(from_info.raw_volume, &entry) {
            return Err(Error::FileAlreadyOpen);
        }

        // Is the destination name taken? Renaming an entry to a different
        // spelling of its own name (a case change) is fine.
        let existing = match &self.open_volumes[volume_idx].volume_type {
        ...
        if entry.attributes.is_directory() && !same_dir {
            ...
            let mut cursor = to_info.cluster;
            loop {
                if cursor == entry.cluster {
                    return Err(Error::Unsupported);
                }
                if cursor == ClusterId::ROOT_DIR {
                    break;
                }
                cursor = match &self.open_volumes[volume_idx].volume_type {
                    VolumeType::Fat(fat) => fat.parent_of_dir(&mut self.block_cache, cursor)?,
                };
            }
        }

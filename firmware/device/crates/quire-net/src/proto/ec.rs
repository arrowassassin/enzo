...
/// One DER INTEGER's content (leading zero allowed) at the front of `d`.
...
pub fn parse_signature(sig: &[u8]) -> Option<(&[u8], &[u8])> {
    ...
    let body = if len < 0x80 { sig.get(2..2 + len)? } else { return None };
...
pub fn verify(curve: Curve, point: &[u8], digest: &[u8], sig: &[u8]) -> bool {
    ...
    // r, s in [1, n).
    let (Some(r_n), Some(s_n)) = (fnn.element(r), fnn.element(s)) else { return false };
    if is_zero(&r_n) || is_zero(&s_n) || super::bignum::strip(r).len() > size || super::bignum::strip(s).len() > size {
        return false;
    }
